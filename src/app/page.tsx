...
function BookTile({ book }: { book: Book }) {
  return (
    ...
    >
      <span className='font-body pb-2 pl-1 text-neutral-700 dark:text-neutral-400 line-clamp-2'>{book.attributes.title}</span>
      <div className='overflow-hidden w-1/3 object-scale-down m-auto'>
        <Image src={getStrapiMedia(book.attributes.cover.data.attributes.url)} width={100} height={200} alt={book.attributes.title} className='w-full rounded-md'/>
      </div>
    </Link>
  );
}
  ...
  return (
    <>
      ...
      <div className='grid grid-cols-2 grid-rows-5 md:grid-rows-3 md:grid-cols-4 gap-2.5 '>
        ...
        {/* First project tile */}
        {latestProjects[0] && (
          <ProjectTile project={latestProjects[0]} />
        )}

        {/* First book tile */}
        {latestBooks[0] && (
          <BookTile book={latestBooks[0]} />
        )}
        ...
        {/* Second project tile */}
        {latestProjects[1] && (
          <ProjectTile project={latestProjects[1]} />
        )}
